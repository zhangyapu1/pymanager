...
1. **启动程序**：双击 `main.pyw` 运行
2. **添加脚本**：点击「添加脚本」或直接将 Python 文件拖入窗口
3. **运行脚本**：双击列表中的脚本即可运行
4. **停止脚本**：点击「停止」按钮或按 `Ctrl+C`

### 脚本市场

点击「脚本市场」进入 GitHub 仓库浏览器：
- **搜索仓库**：输入关键词搜索 GitHub 上的 Python 项目
- **查看 README**：自动加载并渲染项目说明文档
- **翻译 README**：支持 Google、百度、腾讯翻译
- **AI 分析**：使用智谱AI、通义千问或 DeepSeek 分析项目
- **下载脚本**：选择需要的文件一键下载到本地
...
| 操作 | 方法 |
|------|------|
| 添加脚本 | 点击「添加脚本」按钮或拖拽文件 |
| 删除脚本 | 选中后按 `Delete` 或右键菜单「删除」 |
| 重命名 | 右键菜单「重命名」或按 `F2` |
| 移动分组 | 右键菜单「移动到...」 |
| 收藏脚本 | 右键菜单「收藏/置顶」 |
| 批量操作 | `Ctrl+点击` 多选，支持批量删除/移动/导出 |

### 分组管理

- 程序自动扫描 `data/` 目录下的子文件夹作为分组
- 在文件管理器中新建文件夹即可创建新分组
- 分组顺序可在下拉框中拖拽调整
