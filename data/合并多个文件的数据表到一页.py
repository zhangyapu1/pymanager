"""
合并多个文件的数据表到一页 - 将多个 Excel 文件的数据表合并到同一个工作表中。

功能：
    - 多文件汇总：选择多个 Excel 文件，提取指定工作表中指定行范围的数据
    - 工作表筛选：按关键词模糊匹配工作表名称（如输入"销售"匹配"2024年销售报表"）
    - 行范围指定：设置起始行和结束行，只提取该范围内的数据
    - 表头处理：可选包含原始表头或使用默认表头（来源文件、工作表名称、原始行号）
    - 双层进度条：文件级进度 + 工作表内行级进度，实时显示处理状态
    - 支持取消：处理过程中可随时点击取消按钮终止操作
    - 自动列宽：汇总结果自动调整列宽，中文宽度按双倍计算
    - 样式美化：表头行使用蓝色背景白色粗体字，数据行添加细边框
