...
def set_registry_value(key, value_name, value_type, value):
    try:
        ...
        log_info(f"设置注册表: {key}\{value_name} = {value}")
        return True
    except FileNotFoundError:
        log_error("错误: 找不到 reg.exe 命令。请确保在 Windows 环境下运行。")
        return False
    except subprocess.CalledProcessError as e:
        stderr_msg = e.stderr.decode('gbk', errors='ignore') if e.stderr else str(e)
        log_error(f"设置注册表失败: {stderr_msg.strip()}")
        return False
...
def main():
    ...
    if sys.platform != 'win32':
        log_error("错误: 此脚本仅在 Windows 操作系统上支持。")
        return

    log_info("Windows 更新暂停至 2080 年")
    log_info(f"当前运行路径：{os.getcwd()}")
    ...
    log_info("操作完成！Windows 更新已暂停至 2080 年。")
