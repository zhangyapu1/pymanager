...
功能：
    - 多文件合并：选择多个 Excel 文件，每个文件（或每个工作表）保存为独立的工作表
    - 智能命名：单工作表文件使用文件名作为工作表名，多工作表文件使用"文件名_原Sheet名"
    - 名称去重：工作表名重复时自动追加编号（如 "数据_1", "数据_2"）
    - 非法字符过滤：自动替换工作表名中的非法字符（\\ / ? * [ ] :）为下划线
    - 长度限制：工作表名超过 31 字符时自动截断
    - 自动列宽：根据标题和数据内容自动调整列宽（最小5，最大50）
    - 进度条：控制台显示文件处理进度
    - 完成提示音：合并完成后播放提示音（Windows）

注意：
    - 使用 pandas 读取文件，支持 .xlsx / .xls / .csv 格式
    - 输出格式为 .xlsx
    - 不合并数据内容，每个源文件保留在独立的工作表中
