...
功能：
    - 按序号创建：自动生成带编号的文件夹名（如 folder001, folder002...）
      可设置基础名称、起始编号、编号位数、创建数量
    - 添加前缀：为列表中的每个名称添加前缀（如 "项目_" + "A组" → "项目_A组"）
    - 添加后缀：为列表中的每个名称添加后缀（如 "A组" + "_备份" → "A组_备份"）
    - 自定义名称列表：每行输入一个文件夹名，直接创建

安全机制：
    - 自动过滤文件名中的非法字符（<>:"/\\|?*），替换为下划线
    - 去除文件名首尾空格和结尾句点
    - 空名称自动替换为 "unnamed_folder"
    - 已存在的文件夹自动跳过，不会覆盖
    - 创建超过 10000 个文件夹时弹出警告确认
    - 操作前显示预览列表，标记已存在的文件夹
