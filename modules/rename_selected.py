...
辅助函数：
    ...
    _generate_unique_path(dir_name, base_name, extension, original_path)：
        生成不冲突的文件路径
        - 如果目标路径已存在且不是原文件，添加 _1, _2... 后缀
        - 避免覆盖已有文件

安全机制：
    - 路径遍历检查：确保新路径在允许的目录范围内
    - 文件名清理：替换 Windows 非法字符
    - 冲突避免：自动生成唯一路径
    - 重命名失败时输出错误信息
